...
MANUAL_ANGLE_SHIFT = 4  # Set to 0 to disable manual shift
...
def calculate_reflectivity_vs_angle_berreman(wavelength, voltage, n_real_xy, k_xy, n_real_z, k_z, 
    ...
    for angle in angles:
        ...
        # Create permittivity tensors
        # ITO: isotropic material (diagonal tensor with equal elements)
        eps_ito = np.array([
            [n_ito**2, 0,         0        ],
            [0,        n_ito**2,  0        ],
            [0,        0,         n_ito**2]
...
def plot_combined_te_tm_tmm(wavelength, voltage, n_real_te, k_te, n_real_tm, k_tm, 
    ...
    try:
        if os.path.exists('Epoly_35s_Spol.xlsx'):
            df_te = pd.read_excel('Epoly_35s_Spol.xlsx')
            if len(df_te.columns) >= 2:
                external_angles_te = df_te.iloc[:, 0].values
                external_R_te = df_te.iloc[:, 1].values
                mapped_angles_te = -1 * external_angles_te + 273 + shift_te + MANUAL_ANGLE_SHIFT
    ...
    try:
        if os.path.exists('Epoly_35s_Ppol.xlsx'):
            df_tm = pd.read_excel('Epoly_35s_Ppol.xlsx')
            if len(df_tm.columns) >= 2:
                external_angles_tm = df_tm.iloc[:, 0].values
                external_R_tm = df_tm.iloc[:, 1].values
                mapped_angles_tm = -1 * external_angles_tm + 273 + shift_tm + MANUAL_ANGLE_SHIFT
    ...
    try:
        if os.path.exists('Epoly_35s_Spol.xlsx'):
            ...
            if len(df_te.columns) >= 2:
                ...
                mapped_angles_te = -1 * external_angles_te + 273 + shift_te_avg + MANUAL_ANGLE_SHIFT
                sort_idx = np.argsort(mapped_angles_te)
                mapped_angles_te = mapped_angles_te[sort_idx]
                external_R_te = external_R_te[sort_idx]
    ...
    try:
        if os.path.exists('Epoly_35s_Ppol.xlsx'):
            ...
            if len(df_tm.columns) >= 2:
                ...
                mapped_angles_tm = -1 * external_angles_tm + 273 + shift_tm_avg + MANUAL_ANGLE_SHIFT
                sort_idx = np.argsort(mapped_angles_tm)
                mapped_angles_tm = mapped_angles_tm[sort_idx]
                external_R_tm = external_R_tm[sort_idx]
...
class ReflectivityGUI:
    def __init__(self, root):
        ...
        ttk.Checkbutton(exp_frame, text="Show experimental data (Epoly_35s_Spol.xlsx, Epoly_35s_Ppol.xlsx)", 
